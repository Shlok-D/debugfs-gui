...
//bool is_directory(int fd, int ino);
...
void inode_data(struct ext2_inode inode, int fd, int choice, int ino){
    ...
    //     int count = read(fd, &ext2_dirent, sizeof(ext2_dirent));             
    //     if(count == -1){
    //         ;
    //     }
    //     else{
    ...
    //                     printf("%d  %d  %s\n", ext2_dirent.inode, ext2_dirent.rec_len, file_name);
    //                 }
...
//     int pino = 2;
//     while(p == 0){
//         inode = getinode(pino,fd);
//         //for(int i = 0; i < inode.i_blocks; i++){
//         //printf("%d, \n", inode.i_block[0]);
//         inodeno = 0;
//         int n_b = (int) (inode.i_blocks * 512) / block_size;
//         for(int j = 0; j < n_b; j++){
//             lseek64(fd, block_size + (inode.i_block[j]-1)*block_size, SEEK_SET);
//             unsigned int size;
//             int total = inode.i_size;
//             if(inode.i_size > block_size){
//                 total = block_size;
...
//                 if(entry->inode == ino){
//                     inodeno = (int) entry->inode;
//                 }
//                 //printf("%d %u %s\n", (int) entry->inode,entry->file_type, file_name);
